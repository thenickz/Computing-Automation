...
for k in range(1, n):
    x[k] = x[k-1] + h
    y[k] = y[k-1] + h*((4*x[k-1] + 1) * y[k-1]**(1/2))
plt.plot(x, y, color='orange', label = f'Euler Method: h = 0.5')

# Euler Method  h = 0.25
# arrumar aqui colocar mais coisas
h = 0.25
n = int(t_final/h)
for i in range(1, n):
    x[i] = x[i-1] + h
...
plt.plot(xa, ya, color='Green', label = 'Analitic')
plt.plot(x, y, color='blue', 'or', label = f'Euler Method: h = 0.25')
